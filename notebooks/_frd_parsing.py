...
# MAGIC %md
# MAGIC # Shim — the FRD normalizer lives in `src/frdsttm/frd_parsing.py`
# MAGIC
# MAGIC This file re-exports everything from `frdsttm.frd_parsing` so both
# MAGIC consumption paths keep working unchanged:
# MAGIC
# MAGIC - Databricks: `01_frd_ingest` executes `%run ./_frd_parsing`, which runs
# MAGIC   this file's cells in the calling notebook's globals.
# MAGIC - Local scripts: `from _frd_parsing import ...` resolves here because
...
from frdsttm import frd_parsing as _frdsttm_frd_parsing  # noqa: E402

# Re-bind every name (including single-underscore helpers) so `%run` callers
# and `from _frd_parsing import X` both see exactly what the original
# file defined.
globals().update(
    {k: v for k, v in vars(_frdsttm_frd_parsing).items() if not k.startswith("__")}
)
