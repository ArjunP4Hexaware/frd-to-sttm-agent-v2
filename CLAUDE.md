...
folder). It was `…/frd-to-sttm-agent` (no `-app`) until 2026-08-25; that older
folder is **gone** from the workspace (checked 2026-09-16). `README.md`'s
deploy line still names the old path and is **stale**;
`.claude/skills/frd-sttm-agent/SKILL.md` has the current one. Redeploy =
`databricks sync` to the `-app` folder, then `databricks apps deploy`.
