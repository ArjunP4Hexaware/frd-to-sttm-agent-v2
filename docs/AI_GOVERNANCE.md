...
| Model / provider | Anthropic Claude, `claude-opus-5` (job parameter `model` in `resources/frd_sttm_job.yml`), served either by the Anthropic API directly or — with `STTM_LLM_PROVIDER=databricks` (2026-08-24) — by Databricks Foundation Model APIs as `databricks-claude-opus-5`, in which case no Anthropic key or secret scope exists and Databricks bills per token, structured outputs against the Pydantic schema `FrdIngestionSpec` (`src/frdsttm/models.py`, mirrored in `schema/sttm_extraction_schema.json`). Anthropic-only by program decision. **No fine-tuning** (settled; master context §7a). |
