"""
frdsttm.reference_layout — reading an APPROVED STTM workbook for its LAYOUT.

An approved STTM in `reference_sttms` is never a source of content for a
run (the run reads the FRD and the VDD only). It is used for two things:

* ``layout_of`` — which sheet holds which source, where the band / header /
  first data rows are, which column carries which logical field. The render
  writes INTO that layout so a draft looks like the client's own workbooks.
* ``parse_reference_workbook`` — the rows of an approved workbook, used by
  tools/build_vdd_from_sttm.py to derive a worked-example VDD and by the
  index to count columns. Not read by a run.

Both client dialects are supported: sheet_per_table (FILE_DETAILS +
MAPPING-<TABLE> sheets) and single_sheet (one wide sheet with a metadata
block above it).
"""
...
def loose_tokens(s) -> set[str]:
    return {t for t in re.findall(r"[a-z0-9]{4,}", str(s).lower())}


# --------------------------------------------------------------------------- #
# header aliases — both dialects, by NAME not position
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# reading rows (tools + index only)
# --------------------------------------------------------------------------- #
def parse_reference_workbook(path):
    """{"dialect", "feeds": {table_key: {"sheet", "fields", "ref_targets"}}, "meta"}."""
...
def _field_from_row(r, smap, single_sheet):
    get = lambda m, k: _n(r[m[k]]) if k in m and m[k] < len(r) else ""  # noqa: E731
    col = get(smap, "source_column")
    if not col:
        return None
    f = {
        "source_column": col,
        "description": get(smap, "description") or (get(smap, "comment") if single_sheet else ""),
...
def _targets_from_row(r, gmap, dmap):
    get = lambda m, k: _n(r[m[k]]) if k in m and m[k] < len(r) else ""  # noqa: E731
    return {"stage": {k: get(gmap, k) for k in _TARGET_ALIASES},
...
def _parse_sheet_per_table(wb):
    ...
    if "FILE_DETAILS" in wb.sheetnames:
        rows = list(wb["FILE_DETAILS"].iter_rows(values_only=True))
        if rows:
    ...
    for name in wb.sheetnames:
        ...
        for r in ws.iter_rows(min_row=3, values_only=True):
            f = _field_from_row(r, smap, False)
            if f is None:
                continue
            fields.append(f)
            t = _targets_from_row(r, gmap, dmap)
            ref_targets.append(t)
            table_key = table_key or _nl(t["stage"]["table"])
        if table_key:
            feeds[table_key] = {"sheet": name, "fields": fields, "ref_targets": ref_targets}
...
def _parse_single_sheet(wb):
    for name in wb.sheetnames:
        ...
        for r in ws.iter_rows(min_row=header_r + 1, values_only=True):
            ...
            blanks = 0
            fields.append(f)
            ref_targets.append(_targets_from_row(r, gmap, dmap))
        return {"dialect": "single_sheet",
                "feeds": {_nl(name): {"sheet": name, "fields": fields, "ref_targets": ref_targets}},
                "meta": meta}
    return {"dialect": "single_sheet", "feeds": {}, "meta": {}}


# --------------------------------------------------------------------------- #
# layout descriptor — the WRITE-side view
# --------------------------------------------------------------------------- #
...
def layout_of(path) -> dict:
    """Layout descriptor of one approved workbook.

    sheet_per_table → {"dialect", "path", "sheets": {name: {label_row, header_row,
    first_data_row, width, columns, unmapped}}, "file_details", "version_history"}
    single_sheet → {"dialect", "path", "sheet", "label_row", "header_row",
    "first_data_row", "width", "columns", "unmapped", "meta_rows"}
    """
    wb = load_workbook(path, read_only=True)
    out = {"path": str(path)}
    try:
        if any(n.startswith("MAPPING-") for n in wb.sheetnames):
            ...
            if "FILE_DETAILS" in wb.sheetnames:
                ...
                out["file_details"] = {"headers": [_n(h) for h in hdr],
                                       "columns": _map_headers(hdr, _FILE_DETAILS_ALIASES)}
            ...
            if "VERSION_HISTORY" in wb.sheetnames:
                ...
                out["version_history"] = {"headers": [_n(h) for h in hdr],
                                          "columns": _map_headers(hdr, _VERSION_HISTORY_ALIASES)}
            return out
        ...
        for name in wb.sheetnames:
            ...
            for r in range(1, label_r):
                ...
                meta_rows.append({"row": r, "key": k, "field": max(hits)[1] if hits else None})
