...
MODEL = "claude-opus-5"
PRICE_IN, PRICE_OUT = 5.00, 25.00        # $ per 1M tokens, Opus 5
...
def main() -> int:
    ...
    for c in conditions:
        ...
        except Exception as exc:  # noqa: BLE001 — report, do not crash a paid run
            print(f"  {c}: could not parse response ({exc}); raw head: {text[:200]!r}")
            preds = []
